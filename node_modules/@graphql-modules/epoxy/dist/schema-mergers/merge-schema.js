...
function mergeGraphQLTypes(types) {
    var allNodes = types
        ...
        if (utils_1.isGraphQLSchema(type)) {
            ...
            var allTypesPrinted = Object.keys(typesMap_1).map(function (key) { return typesMap_1[key]; }).map(function (type) { return type.astNode ? graphql_1.print(type.astNode) : null; }).filter(function (e) { return e; });
            var directivesDeclaration = schema.getDirectives().map(function (directive) { return directive.astNode ? graphql_1.print(directive.astNode) : null; }).filter(function (e) { return e; });
            var printedSchema = directivesDeclaration.concat(allTypesPrinted).join('\n');
